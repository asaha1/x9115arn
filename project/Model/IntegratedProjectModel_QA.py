...
class Model(object):
    ...
    def eval(self):
        return sum(self.getObjs(self.val))

    def getObjs(self, ind = None):
        pass
...
class Integrated_project_model_qa(Model):
    def __init__(self):
        self.decs = 17
        self.objs = 2
        self.val = [0 for _ in range(17)]
        self.domMin = [x[0] for x in Utility.bounds]
        self.domMax = [x[1] for x in Utility.bounds]
        self.setDecs()

    def getObjs(self, aux_list):
